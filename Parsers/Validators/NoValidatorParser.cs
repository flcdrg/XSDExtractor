...
namespace JFDI.Utils.XSDExtractor.Parsers.Validators {
  ...
  public class NoValidatorParser : ValidatorAttributeParser {
    ...
    public override XmlSchemaSimpleType GetSimpleType(string attributeDataType) {
      ...
      XmlSchemaSimpleType retVal = new XmlSchemaSimpleType();

      // <xs:restriction base="<whatever xs:datatype is required>" />
      XmlSchemaSimpleTypeRestriction dataTypeRestriction = new XmlSchemaSimpleTypeRestriction();
      dataTypeRestriction.BaseTypeName = new XmlQualifiedName(attributeDataType);
      ...
      if(property.PropertyType.IsEnum) {
        ...
        foreach (string enumValue in Enum.GetNames(property.PropertyType)) {
          XmlSchemaEnumerationFacet enumFacet = new XmlSchemaEnumerationFacet();
          enumFacet.Value = enumValue;
